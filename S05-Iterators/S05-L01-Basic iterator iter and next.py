...
class Combinations:
    def __init__(self, product, promotions, customers):
    ...
    def __next__(self):
        ...
        if self.current_product >= len(self.product):
            self.current_product = 0
            raise StopIteration

        item_to_return = ["{} - {} - {}".format(self.product[self.current_product], self.promotions[self.current_promotion], self.customers[self.current_customer])]
